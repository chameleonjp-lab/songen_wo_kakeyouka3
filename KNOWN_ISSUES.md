...
| Pages設定待ち | merge後のDeploy to GitHub Pages run #2（commit `400998196`）も `actions/configure-pages@v5` の `Get Pages site failed: Not Found` で停止。2026-08-20時点で Pages API と公開URLが404で、リポジトリのPagesが未有効化。次回からworkflow先頭のPages API preflightで同じ状態を早く明示する | Repository Settings → Pages → Build and deployment → Source を **GitHub Actions** に設定してから `Deploy to GitHub Pages` を再実行 |
