...
弱攻撃中に次の弱／強を短い入力バッファへ入れると、弱→弱→強または弱→強のルートになります。3回目の弱入力は次のコンボへ持ち越しません。ジャストガードはガード開始直後の0.16秒、成功後のカウンター受付は0.72秒です。攻撃相殺の判定窓は攻撃進行度0.40〜0.68です。

## スマホ／iPhone

画面下のDパッドを長押しして移動し、右側の「防御」「回避」「狙い」「弱攻撃」「強攻撃」「怒破」をタップします。Dパッドとカメラ用の空き面は分離されています。空き面のスワイプで視点を動かし、カメラはプレイヤー背後の範囲から外れません。
