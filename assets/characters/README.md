...
尊厳が0になったときに、ガチョウや6種類の動物から共通状態へ切り替えるためのモデルです。
...
この2ファイルは、既存の動物6種類のGLBを変更せずに使える共通状態モデルです。現在のゲーム側は尊厳0を条件に共通smoothモデルへ切り替え、親rootの位置・向き、体力、怒気、戦闘進行を維持します。切替前後とも実測19骨・44アニメーションです。
